  ...
  return (
    <div className={`relative inline-block ${className}`} ref={dropdownRef}>
      <button
        ...
        className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-zinc-100/80 dark:bg-zinc-800/80 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-200 border border-zinc-200/80 dark:border-zinc-700 text-xs font-semibold transition-all active:scale-95"
        title="Change Indian Language"
      >
        <span className="text-sm">{currentLanguage.flag}</span>
        <span className="font-bold hidden sm:inline">{currentLanguage.native}</span>
        <span className="text-[10px] text-zinc-400 dark:text-zinc-500 uppercase">
          ({currentLanguage.code})
        </span>
        <FiChevronDown className={`w-3 h-3 text-zinc-400 transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`} />
      </button>

      {isOpen && (
        <div className="absolute right-0 mt-2 w-72 max-h-96 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-2xl py-2 z-50 animate-slide-down flex flex-col">
          <div className="px-3 pb-2 border-b border-zinc-100 dark:border-zinc-800/80 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">
                {t("selectLanguage", "Select Indian Language")}
              </span>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-1.5 text-xs rounded-lg bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-zinc-900 dark:text-white placeholder-zinc-400 outline-none focus:border-blue-500"
                autoFocus
              />
            </div>
          </div>
